...
### 3.2  在消息循环中进行判断，如果客户区要求重绘，则执行Game_Paint()函数
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam){
	...
	case WM_PAINT://如果客户区重绘信息
		g_hdc = BeginPaint(hWnd,&paintStruct);//开始绘画
		Game_Paint(hWnd);//绘画核心代码
		EndPaint(hWnd,&paintStruct);//结束绘画
		ValidateRect(hWnd,NULL);//该函数更新指定窗口的无效矩形区域，使之有效
		break;
	case WM_DESTROY:
         //3.资源清理
		Game_CleanUp(hWnd);
