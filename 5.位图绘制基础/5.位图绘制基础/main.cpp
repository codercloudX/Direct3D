...
HWND hWnd = NULL;
HBITMAP g_hBitmap[2] = {0};//定义一个位图句柄
...
void Game_Paint(HWND hWnd){
	...
	//----------------------【位图绘制4.进行贴图】------------------------//
	BitBlt(g_hdc,0,0,WINDOW_WIDTH,WINDOW_HEIGHT,g_mdc,0,0,SRCCOPY);//采用BitBlt方法进行贴图，参数设置为窗口大小

	//----------------------【位图绘制3.选用位图对象】------------------------//
	SelectObject(g_mdc, g_hBitmap[1]);

	//----------------------【位图绘制4.进行贴图】------------------------//
	BitBlt(g_hdc, 400, 0, WINDOW_WIDTH , WINDOW_HEIGHT , g_mdc, 0, 0, SRCCOPY);//采用BitBlt方法进行贴图，参数设置为窗口大小
}
